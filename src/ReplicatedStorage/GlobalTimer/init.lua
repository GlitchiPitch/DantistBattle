...
local function initialize()
    local runServiceFunction = RunService:IsServer() and RunService.Heartbeat or RunService.RenderStepped
    
	table.insert(_connections, event.Event:Connect(eventConnect))
	table.insert(_connections, runServiceFunction:Connect(onRender))
end
